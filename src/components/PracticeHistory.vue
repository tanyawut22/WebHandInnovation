  ...
  <div>
    ...
    <div class="container">
      ...
      <div>
        ...
        <br>
          <a style="font-size: 20px;"><strong>โหมดการฝึก: </strong>{{Modetext}} ( {{Mode}} ) เป็นเวลา {{ModeSec}} วินาที</a>
        <br>
...
export default {
    ...
    methods: {
        ...
        getMode(selectedStudentIDValue){
            const q = query(collection(firestoredb, "Students", selectedStudentIDValue, "PracticeHistory"));
            onSnapshot(q, (querySnapshot) => {
                querySnapshot.forEach((doc) => {
        ...
        getChart(selectedStudentIDValue, selectedModeValue){
          // this.DocIDSize()
              onSnapshot(doc(firestoredb, "Students", selectedStudentIDValue, "PracticeHistory", selectedModeValue), (doc) => {
    ...
    watch: {
            selectedStudentIDValue: {
                handler(value) {
                    ...
                    const q = query(collection(firestoredb, "Students", value, "PracticeHistory"));
                    onSnapshot(q, (querySnapshot) => {
                        querySnapshot.forEach((doc) => {
                            this.SelectModeArray.push({'Id': doc.id, 'Name': "ครั้งที่ "+doc.id});
                        });
